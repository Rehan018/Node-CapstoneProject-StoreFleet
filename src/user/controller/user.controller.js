...
export const resetUserPassword = async (req, res, next) => {
  ...
  try {
    const hashedToken = crypto
      ...
      .digest("hex");
    const userForPasswordReset = await findUserForPasswordResetRepo(hashedToken);
...
export const updatePassword = async (req, res, next) => {
  ...
  try {
    ...
    const userToUpdatePassword = await findUserRepo({ _id: req.user._id }, true);
    const passwordMatch = await userToUpdatePassword.comparePassword(currentPassword);
    if (!passwordMatch) {
